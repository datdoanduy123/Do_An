    ...
    public class TaiLieuDuAnService : ITaiLieuDuAnService
    {
        ...
        public async Task<bool> ProcessWithAiAsync(int taiLieuId)
        {
            ...
            try
            {
                ...
                bool success = false;
                string ext = taiLieu.FileType.ToLower();
                if (ext == ".docx")
                {
                    success = await ProcessWordAsync(taiLieu);
                }
                else if (ext == ".xlsx")
                {
                    success = await ProcessExcelAsync(taiLieu);
                }
        ...
        private async Task<bool> ProcessWordAsync(TaiLieuDuAn taiLieu)
        {
        ...
        private async Task<bool> ProcessExcelAsync(TaiLieuDuAn taiLieu)
        {
            using (var stream = new System.IO.FileStream(taiLieu.FilePath, System.IO.FileMode.Open, System.IO.FileAccess.Read))
            {
                ...
                for (int i = 1; i <= sheet.LastRowNum; i++)
                {
                    ...
                    string moduleName = GetCellText(moduleIdx);
                    if (!string.IsNullOrEmpty(moduleName)) currentModuleName = moduleName;

                    var title = GetCellText(titleIdx);
                    var desc = GetCellText(descIdx);
                    var typeStr = GetCellText(typeIdx);
                    var skillStr = GetCellText(skillIdx);
                    var priorityStr = GetCellText(priorityIdx);
                    var dependencyStr = GetCellText(depIdx);
                    var positionStr = GetCellText(sttIdx);
                ...
                for (int idx = 0; idx < tasksInTable.Count; idx++)
                {
                    ...
                    if (string.IsNullOrEmpty(depStr) || depStr.Trim() == "0" || depStr.Trim() == "-") continue;
